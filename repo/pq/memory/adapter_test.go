...
func TestAdapter(t *testing.T) {
	...
	now := time.Now()
	status, err = repo.Push(ctx, topic, 4, jobq.ID(2), now.Add(4*time.Second))
	require.NoError(err)
	require.Equal(jobq.JobStatusDelayed, status)

	status, err = repo.Push(ctx, topic, 2, jobq.ID(3), now.Add(2*time.Second))
	require.NoError(err)
	require.Equal(jobq.JobStatusDelayed, status)

	status, err = repo.Push(ctx, topic, 3, jobq.ID(4), now.Add(3*time.Second))
	require.NoError(err)
	require.Equal(jobq.JobStatusDelayed, status)
